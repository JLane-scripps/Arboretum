...
class RedBlackTree:
    ...
    def __init__(
        ...
        data: Any = None,      # if limited to 1 value like this, mz will be used for the most distinction
        color: int = 0,
    ...
    def insert(self, label: float, data: Any) -> RedBlackTree:  #modify to pass in whole PSM
        ...
        if self.label == label:
            ...
            if self.left:
                self.left.insert(label, data)
            else:
                self.left = RedBlackTree(label, data, 1, self)
                self.left._insert_repair()
        else:
            if self.right:
                self.right.insert(label, data)
            else:
                self.right = RedBlackTree(label, data, 1, self)
    ...
    def remove(self, label: float) -> RedBlackTree:
    ...
    def __contains__(self, label: float) -> bool:
    ...
    def search(self, sought: float) -> RedBlackTree | None:
        """Search through the tree for label, returning its node if
        it's found, and None otherwise.
        This method is guaranteed to run in O(log(n)) time.
        """

        if self.label == sought:
            return self
        elif self.label is not None and sought > self.label:
    ...
    """
    search_bounded takes in a lower bound and an upper bound 
    and uses that to return a list composed of RedBlackTree nodes
    """
    def search_bounded(self, lower_bound, upper_bound) -> list[RedBlackTree]:
        matches = []

        #error case: parent node is None
        if self.label is None:
            return matches

        #case 1: parent node is a match, search left AND right children
        if lower_bound <= self.label <= upper_bound:
            if self.left is not None:
                matches.extend(self.left.search_bounded(lower_bound, upper_bound))
            if self.right is not None:
                matches.extend(self.right.search_bounded(lower_bound, upper_bound))
            matches.append(self)

        #case 2: parent node is a match but is upper bound, or is larger, search left only
        elif self.label >= upper_bound:
            if self.left is not None:
                self.left.search_bounded(lower_bound, upper_bound)

        #case 3: parent node is a match but is lower bound, or is larger, search right only
        elif self.label <= lower_bound:
            if self.right is not None:
                self.right.search_bounded(lower_bound, upper_bound)

        return matches

    def floor(self, label: float) -> int | None:
    ...
    def ceil(self, label: float) -> int | None:
